...
## load packages
lib <- c("MODIS", "Rsenal", "rgdal", "doParallel", "latticeExtra", "reset")
Orcs::loadPkgs(lib)
...
lst <- lapply(c("MOD09Q1", "MYD09Q1"), function(product) {
  ...
  # 
  #   ## loop over layers
  #   lst_out <- foreach(j = 1:nlayers(i), .packages = "raster") %dopar%
  ...
  #     raster::overlay(i[[j]], rst_crp[[3]][[j]], fun = function(x, y) {
  #       id <- sapply(y[], function(k) {
  #         bin <- Rsenal::number2binary(k, 16, TRUE)
  ...
  lst_qc3 <- foreach(i = lst_qc2) %do% {
    raster::stack(foreach(j = unstack(i), .packages = "raster", 
...
lst_ndvi_cmb <- foreach(i = 1:nlayers(lst_fill[[1]]), .packages = "raster") %dopar%
  overlay(lst_fill[[1]][[i]], lst_fill[[2]][[i]], 
          fun = function(...) max(..., na.rm = TRUE), 
          filename = paste0(dir_cmb_ndvi, "/", fls_cmb_ndvi[i]), 
          format = "GTiff", overwrite = TRUE)
...
lst_savi_cmb <- foreach(i = 1:nlayers(lst_fill[[3]]), .packages = "raster") %dopar% 
  overlay(lst_fill[[3]][[i]], lst_fill[[4]][[i]], 
          fun = function(...) max(..., na.rm = TRUE), 
          filename = paste0(dir_cmb_savi, "/", fls_cmb_savi[i]), 
          format = "GTiff", overwrite = TRUE)
